...
//System
    ...
    public GP(int seed, int epochs,int size,int depth,double gSplit, double tSplit,Tree iniTree,double copies, double mutants, double newTrees){
        this.seed = seed;
        this.epochs = epochs;
        this.size = size;
        bestTree = iniTree.copyTree();
        reader = new CSVReader();
        reader.extractLarge();
        bestTree.varList = reader.variable;
        bestTree.resetVarNames();
        population = new Population(size, this.seed, depth,reader,bestTree,copies,mutants,newTrees);
        this.random = new Random(this.seed);
        goSplit = gSplit;
        testSplit = tSplit;
        positionCounter = 0;
    ...
    public void test(){
            ...
            for(int j = testStart; j < reader.dataAmount; j++){
                ...
                bestTree.calculateFitness();
                //System.out.println("Target score : " + bestTree.target + "  GetScore : " + bestTree.calculateScore() );
                //System.out.println("Calc fitness : " + bestTree.calculateFitness() + "  AvgFit : " + bestTree.avgFitness );
                //System.out.println("----------------");
            }
            //System.out.println("Best fitness : " + bestTree.avgFitness);
            //System.out.println("Best tree : " + bestTree.toString());
