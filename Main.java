...
public class Main {
    public static void main(String[] args) {
        ...
        int seed = 1458000029 ; // This is the best seed as stated in the document
      
        
       
        singleTransferRun(seed);
        //transferTest();
    ...
    public static void testCSVReader() {
        ...
        reader.extractSmall();
        for (int j = 0; j < reader.variable.length; j++) {
            System.out.println(j + " " + reader.variable[j]);
        }
    ...
    public static void singleTransferRun(int seed) {

        //seed = 1458000029;
        double params[] = {seed,200,200,6,0.7,0.7};
        GPTransffered g = new GPTransffered(params);
        g.trainGP();
        double params2[] = {seed,250,250,7,0.7,0.7};
        g.transferAndCreate(params2, 0.25, 0.50, 0.25);
        g.trainGPT();
        g.testGPT();
    ...
    public static void transferTest() {
        ...
        bTree = null;
        bSeed = 0;
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("TransferTest.txt", true))) {
            writer.write("TransferTest :" + "\n");
        } catch (IOException e) {
            System.err.println("Error writing to seed file: " + e.getMessage());
        }
        ...
        for (int i = 0; i < 10; i++) {
            ...
            g.trainGPT();
            g.testGPT();
            System.out.println( "\n" + "---------" + "\n" + "seed: " + seed + " tree: " + g.bestTree.toString() + " aveF: " + g.bestTree.avgFitness
                    + "\n" + "---------" + "\n");

            try (BufferedWriter writer = new BufferedWriter(new FileWriter("TransferTest.txt", true))) {
                writer.write(i + "  Seed: " + +seed + " Best tree AveFitness: " + g.bestTree.avgFitness + " Tree Form:" + g.bestTree.toString() + "\\" + "\\"
                        + "\n");
            } catch (IOException e) {
                System.err.println("Error writing to seed file: " + e.getMessage());
            }
